...
func TestMongoDBValidator_Admit(t *testing.T) {
	for _, c := range cases {
		t.Run(c.testName, func(t *testing.T) {
			...
			validator.client = fake.NewSimpleClientset(storageClass())
...
var cases = []struct {
	...
	{"Edit MongoDB Spec.Version By User",
		requestKind,
		"foo",
		"default",
		admission.Update,
		userIsHooman(),
		editSpecVersion(sampleMongoDB()),
		sampleMongoDB(),
		false,
		false,
	},
	{"Edit MongoDB Spec.Storage By User",
		requestKind,
		"foo",
		"default",
		admission.Update,
		userIsHooman(),
		editSpecStorage(sampleMongoDB()),
		sampleMongoDB(),
		false,
		false,
	},
...
func storageClass() *storageV1beta1.StorageClass {
...
func editSpecVersion(old api.MongoDB) api.MongoDB {
	old.Spec.Version = "3.6"
	return old
}

func editSpecStorage(old api.MongoDB) api.MongoDB {
	old.Spec.Storage.StorageClassName = types.StringP("new-class")
	return old
}
